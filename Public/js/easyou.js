...
function search(url) {
    var key = $("#key").val();
    window.location.href = root + url + key;
}
//跳转
function doTurn(url) {
...
//GET请求
function doGet(url, rtUrl) {//请求地址和完成后跳转地址
...
//弹窗
function eyAlert(content, title, s) {
    if (!title) {
        var title = '提示';
    }
    var d = dialog({
        ...
        onremove: function () { //销毁事件
            if (!s) {
                window.location.href = window.location.href;
            }else if(s=='pjax'){
                artClose();
                pjaxJump(window.location.href);
            }
        },
    });
    d.show();
...
//相册密码验证
function check(id) {
    $.get(root + '/index.php/Admin/Login/photo/id/' + id, function (data) {
        eval('data=' + data);
        if (data.status == 0) {
            var $modal = $('#my-prompt');
            $modal.modal({
                ...
                onConfirm: function (e) {
                    var pwd = e.data;
                    var url = '/index.php/Admin/Login/photo';
                    var data = {id: id, pwd: pwd};
                    $.post(root + url, data, function (data) {
                        eval('data=' + data);
                        if (data.status == 1) {
                            $modal.modal('close');
                            pjaxJump(root+'/photo/' + id + '.html');
                        } else {
                            eyAlert(data.msg, false, 1);
                        }
                    });

                },
                onCancel: function (e) {
                    eyAlert('没有输入任何数据', false, 1);
                }
            });
        } else {
            pjaxJump(root+'/photo/' + id + '.html');
...
function getComment() {
    var pid = $("#pid").val();
    if($("#pageNum").data("p")){
        var p = $("#pageNum").data("p");
    }else{
        p =2;
    }
    var div = $("#comment");
    var url = "/index.php/Home/Comment/getList/id/" + pid + "/p/" + p;
    $.get(root + url, function (data) {
        eval('data=' + data);
        if (data.body != "") {
            for (var p in data.body) {//遍历数组，P为索引
                ...
                if(data.body[p].nodeList){
                    str+='<div class="author"><span class="am-blog-auth"><a href="'+data.body[p].nodeList.address+'">@'+data.body[p].nodeList.name+'</a>:'+data.body[p].nodeList.comment+'</span></div>';
                }
                str+=data.body[p].comment + '</div> </div> </article>';
                div.append(str);
            }
        } else {
            eyAlert('没有更多了', false, 1);
        }
    });
    $("#pageNum").data("p",parseInt(p)+1);//页码加1
}
//弹出div
function showDiv(id,title,nid){
    if(nid){
        $("#nid").val(nid);
    }
    var respond=$("#"+id);
...
//弹出图片
function popPicture(id){
    var src=$("#"+id).attr('src');
    var title=$("#"+id).attr('title');
    $("#my-popup-title").text(title);
...
$(document).on('submit', '.pjax_form', function (event) {
    $.pjax.submit(event, '#pjax_content', {fragment: '#pjax_content', timeout: 6000});
});